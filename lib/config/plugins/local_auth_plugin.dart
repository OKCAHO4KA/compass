    ...
    try {
      final bool didAuthenticate = await auth.authenticate(
          ...
          options: AuthenticationOptions(
              // biometricOnly: true - только вход по биометрике!
              // biometricOnly: false - можем по ПИН разблокировать
              biometricOnly: biometricOnly));
      return (didAuthenticate, didAuthenticate ? 'Authenticado' : ' Bloqueado');
    } on PlatformException catch (e) {
      print('e');

      if (e.code == auth_error.notEnrolled) {
        return (false, 'No hay biométricos enrollados');
      }
      ...
      if (e.code == auth_error.passcodeNotSet) {
        return (false, 'No hay el PIN configurado');
      }
